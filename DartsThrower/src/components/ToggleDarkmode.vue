...
export default {
  ...
  mounted() {
    ...
    if (darkModeOn) {
      this.darkMode = JSON.parse(darkModeOn);
...
<template>
  <div
    class="flex flex-col justify-center ml-3 focus-within:outline outline-2"
    @click="toggleDarkMode"
  >
    <input type="checkbox" name="light-switch" class="light-switch sr-only" />
    <label class="relative cursor-pointer p-2" for="light-switch">
      <svg
        class="dark:hidden scale-[2]"
        width="16"
        height="16"
      ...
      <svg
        class="hidden dark:block scale-[2]"
        width="16"
        height="16"
        xmlns="http://www.w3.org/2000/svg"
      >
